...
def consulta_tiempo ():
    ...
    r = requests.get('http://localhost:3000/api/dashboards/uid/dtJuJGsiz', auth=('admin', '12344321'))    
    print ''
    print 'Estado:',r.status_code
    #hacer un if para la instruccion anterior
    a = r.json()
    dato = a['dashboard']['panels'][0]['targets'][0]['groupBy'][0]['params'][0]
    print 'dato:',dato
    tiempo = 2
    
    if dato == '$__interval':
        dato = a['dashboard']['panels'][0]['interval']
        print 'dato: ',dato
